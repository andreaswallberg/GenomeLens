 ...
 */
function updateCanvasTitle(plotSpec) {
    try {
        if (window.canvas_num !== 0) {
            const xSelector = document.getElementById('columnSelectorX_0')
            const ySelector = document.getElementById('columnSelectorYLeft')
            let xCol = xSelector ? xSelector.options[xSelector.selectedIndex]?.textContent : ''
            let yCol = ySelector ? ySelector.options[ySelector.selectedIndex]?.textContent : ''
            if (plotSpec && typeof plotSpec.title !== 'undefined') {
                plotSpec.title = `${yCol} vs ${xCol}`
            } else if (plotSpec && plotSpec.views && plotSpec.views[0]) {
                plotSpec.views[0].title = `${yCol} vs ${xCol}`
            }
        } 
    } catch (e) {
        console.warn('updateCanvasTitle error:', e)
    }
}